...
**Real, currently open, disclosed blocking bug: a syscall-entry/exit race under genuine concurrent load.** Once the terminal's interactive poll loop (`SYS_IPC_TRY_RECEIVE`, syscall 12, called in a tight loop) runs concurrently with `keyboard_driver`'s own syscalls (reading a scancode, routing it via syscall 13, logging it) — the FIRST real high-frequency multi-thread concurrent-syscall scenario this kernel has ever exercised — the terminal reliably crashes (a real, reproduced ring-3 write #PF at an address just past its own mapped stack, right after the first real routed keystroke is consumed). Root-caused, not just observed: `syscall.rs`'s entry/exit stub stashes the calling thread's user RSP in a PER-CORE (not per-thread) GS-relative scratch slot (`gs:[8]`) and deliberately re-enables interrupts (`sti`) mid-dispatch so blocking syscalls (`ipc::send`/`receive`) don't deadlock the timer — but that same preemption window means a SECOND thread's own syscall entry on the same core can overwrite that scratch slot before the first thread's exit reads its own value back out, handing it the wrong RSP on return. `syscall.rs`'s own module doc already documents fixing an earlier, narrower version of this exact bug class (the GS-base-active toggle race) — this is a real, distinct escalation of the same underlying design tension (a per-core, not per-thread, scratch resource, safe only as long as no two threads' syscalls actually interleave) exposed for the first time by this app's own tight polling loop. Not fixed in this pass — the real fix (making the scratch genuinely per-thread, or re-architecting blocking syscalls to not need mid-dispatch preemption) is real, disclosed, separate kernel work, not a quick patch. `terminal_emulator`'s own code is otherwise complete and correct; `scripts/test-terminal.ps1` exists but is NOT wired into the regression suite while this blocks it, so as not to report a false green.

**Not yet started:** reading a manifest+ELF pair off a real on-disk package store (disclosed above), higher-level syscall wrappers beyond the raw ABI, the remaining three reference apps (text editor, file manager, network client — deliverable 4), and app update/versioning (deliverable 5). None of Phase 13's four exit criteria are formally met yet, though the mechanism behind the first ("installing an app grants exactly its manifest's declared capabilities, nothing ambient... demonstrated adversarially") is now real and proven against a synthetic demo, a genuine simple app (`installer_demo.rs`), and a genuine, more complex app (`terminal_emulator`) all three.
