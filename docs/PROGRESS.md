...
**Status of Phase 10's four exit criteria:** revocation (3) and crash-and-restart (4) are now met, both independent of the TCP bug. External HTTP GET (1) and two-instance TCP exchange (2) remain blocked — the real TCP client is written and ready the moment the aggregate-construction bug is root-caused.

**Not yet started:** DNS as a working capability-scoped client (deliverable 3, blocked on the aggregate-construction bug), loopback/multi-NIC routing (deliverable 5), and binding the `Socket` capability's revocation to an actual live `netstack_driver` connection (real follow-up once TCP itself works — the capability-lifecycle mechanism proven above generalizes directly, nothing new to design).
