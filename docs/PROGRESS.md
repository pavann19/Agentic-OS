...
**Honest, disclosed, currently-open gap — stated plainly, not hidden:** the UDP/DNS path (`udp_build`, `dns_resolve`, `dns_build_query`, real and compiled) is disabled at its one call site. Calling it reaches a genuine, unresolved runtime page fault (a data read from address 0) that survives the `.cargo/config.toml` fix above — this session's investigation ruled out, each with real evidence, not guesses: `str::split`'s pattern-matching machinery, `[u8]::copy_from_slice`'s `memcpy` lowering, the missing build-config files (a real, separate bug, fixed, but not the whole story), stack size (tested up to 128KB), LTO on/off, and forced inlining. Not yet root-caused. The identical real Ethernet/ARP/IPv4/checksum machinery the ICMP path already exercises successfully, every run, makes this specifically a UDP/DNS-path issue, not a general one. Left disabled and documented at the call site (`netstack_driver/src/main.rs`) rather than shipped broken or silently deleted — real follow-up work for the next session.

**Not yet started:** TCP (deliverable 2's own "real, spec-compliant" requirement — congestion control, retransmission, the actual state machine), the capability-scoped socket abstraction (deliverable 1), DNS as a working capability-scoped client (deliverable 3, blocked on the UDP/DNS gap above), crash-and-restart demonstrated specifically for this process (deliverable 4 — the mechanism is wired, per Phase 9.5a reuse above, but no fault-injection demo has been built for it yet, unlike AHCI's), and loopback/multi-NIC routing (deliverable 5). None of Phase 10's four exit criteria (external HTTP GET, two-instance TCP exchange, mid-connection revocation, network-stack crash-and-restart) are met yet.
